...
import dev.nebalus.libary.logging.handler.LogHandlerInterface;
...
public class LogChannel implements ResettableInterface
{
	...
	public boolean addRecord(LogRecord logRecord, boolean isAsync)
	{
		...
		for (LogHandlerInterface handler : handlers) {
			...
			// once the logRecord is initialized, send it to all handlers as long as the bubbling chain is not interrupted
			try {
				isHandled = true;
