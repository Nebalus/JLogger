...
public class Logger extends LogChannel {
	...
	protected void dispatch(LogLevel logLevel, String[] labels, String msg, Object[] messageFormatArgs, boolean isAsync)
	{
		String msgFormatted = String.format(msg, messageFormatArgs);
		LogRecord logRecord = new LogRecord(new Date(), logLevel, channelName, Thread.currentThread().getName(), labels, msgFormatted);
	...
	{
		LogRecord logRecord = new LogRecord(new Date(), logLevel, channelName, Thread.currentThread().getName(), labels, throwable);
		super.addRecord(logRecord, isAsync);
	}
